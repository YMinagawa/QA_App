...
public class QuestionSendActivity extends AppCompatActivity implements View.OnClickListener, DatabaseReference.CompletionListener{
    ...
    //Activity内で複数のパーミッションの許可のダイアログを出すことがある場合、
    //複数のActivityから戻ってくることがある場合に識別するための値
    ...
    private int mGenre; //ジャンルを保存する
    private Uri mPictureUri; //カメラで撮影した画像を補zんするURI
    ...
    //Intent連携から戻ってきた時に画像を取得しImageViewに設定する
    //dataがnullかdata.getDATA()の場合はカメラで撮影したときなので画像の取得にmPictureUriを使用します。
    //ContentResolverクラス、InputStreamクラス、BitmapFactoryクラスを使ってURIからBitmapを作成する。
    @Override
    ...
    //onClickメソッドでは添付画像を選択、表示するImageViewをタップした時と、投稿ボタンをタップした時の処理を行う。
    //まずAndroid6.0以降か否かで処理を分ける。
    //6.0以降であればcheckSelfPermissionメソッドで外部ストレージへの書き込みが許可されているか確認する。
    //許可されていなければrequestPermissionメソッドで許可ダイアログを表示させる。
    //Android5.0以前の場合はパーミッションの許可状態を確認せずにshowChooserメソッドを呼び出す。
    @Override
    public void onClick(View v) {
        if(v==mImageView){
            ...
            DatabaseReference genreRef = dataBaseReference.child(Const.ContentPATH).child(String.valueOf(mGenre));
            ...
            //添付画像が設定されていれば画像を取り出してBASE64をエンコードする
            if(drawable != null){
    ...
    //onRequestPermissionResultメソッドは許可ダイアログでユーザが選択した結果を受け取る。
    //grantResults[0] == PackageManager.PERMISSION_GERANTED
    //とすることで許可したかどうかを判断することができる。許可された場合にはshowChooserメソッドを呼び出す。
    @Override
    ...
    //showChooserメソッドではギャラリーから選択するIntentとカメラで撮影するIntentを作成して、
    //さらにそれらを選択するIntentを作成してダイアログを表示する。
    private void showChooser(){
        ...
        values.put(MediaStore.Images.Media.MIME_TYPE, "image/jpege");
    ...
    //Firebaseへの保存が完了したらfinsih()メソッドを呼び出してActivityを閉じます。
